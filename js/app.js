...
function setME(val) {
  getTodayLog().me = val;
  setButtonGroup('me-tracker', val, { none: [0, 'red'], partial: [1, 'active'], moderate: [2, 'active'], strong: [3, 'green'], full: [4, 'green'] });
  renderMEDisplay();
  renderMECalendar();
  saveState();
}
...
function getMELabel(value) {
  return ({ none: 'None', partial: 'Partial', moderate: 'Moderate', strong: 'Strong', full: 'Full' })[value] || 'Not logged';
}

function renderMEDisplay() {
  const el = document.getElementById('me-display');
  if (!el) return;
  const me = getTodayLog().me;
  el.innerHTML = `<span class="tag ${me === 'strong' || me === 'full' ? 'green' : me === 'none' ? '' : 'blue'}">${getMELabel(me)}</span>`;
}

function renderMECalendar() {
  const grid = document.getElementById('me-calendar-grid');
  const label = document.getElementById('me-calendar-month');
  if (!grid) return;
  ...
  const todayKey = toDateKey(today);
  const dayNames = ['S','M','T','W','T','F','S'];
  if (label) label.textContent = today.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const me = log?.me;
    let cls = 'calendar-day';
    let title = `Day ${day} - Morning erection ${getMELabel(me)}`;
    if (me) cls += ` me-${me}`;
    if (key === todayKey) cls += ' today';
    cells.push(`<button class="${cls}" type="button" title="${title}" aria-label="${title}" onclick="openCalendarDetail('${key}')">${day}</button>`);
  }
...
function refreshTodayControls() {
  const log = getTodayLog();
  setButtonGroup('me-tracker', log.me, { none: [0, 'red'], partial: [1, 'active'], moderate: [2, 'active'], strong: [3, 'green'], full: [4, 'green'] });
...
function openCalendarDetail(key) {
  ...
  setButtonGroup('detail-me', log.me, { none: [0, 'red'], partial: [1, 'active'], moderate: [2, 'active'], strong: [3, 'green'], full: [4, 'green'] });
...
function setDetailME(value) {
  if (!selectedDetailKey) return;
  const log = getLogByKey(selectedDetailKey);
  log.me = value;
  setButtonGroup('detail-me', value, { none: [0, 'red'], partial: [1, 'active'], moderate: [2, 'active'], strong: [3, 'green'], full: [4, 'green'] });
  if (selectedDetailKey === getTodayKey()) refreshTodayControls();
  saveState();
  refreshDashboard();
}
...
// ══ SERVER SYNC ══════════════════════════════════════
const SERVER_SYNC_ENDPOINT = 'api/sync.php';
let serverSyncTimer = null;
let hasLoadedServerData = false;

function getClientId() {
  let id = localStorage.getItem('rewire_client_id');
  if (!id) {
    id = (crypto.randomUUID ? crypto.randomUUID() : `client-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    localStorage.setItem('rewire_client_id', id);
  }
  return id;
}
...
function queueServerSync() {
  ...
  serverSyncTimer = setTimeout(syncToServer, 1800);
}
  ...
  try {
    const res = await fetch(SERVER_SYNC_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        payload: exportPayloadObject(),
      }),
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    setServerSyncStatus(`Server sync: saved at ${new Date().toLocaleTimeString()}.`);
  } catch {
    setServerSyncStatus('Server sync: offline or backend endpoint not available. Local data is still saved.');
  }
}
...
async function loadFromServerOnce() {
  ...
  try {
    const res = await fetch(`${SERVER_SYNC_ENDPOINT}?clientId=${encodeURIComponent(getClientId())}`);
    if (res.status === 404) {
      setServerSyncStatus('Server sync: no server data yet.');
      return;
    }
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const parsed = await res.json();
    const importedState = parsed.state || parsed.payload?.state;
    if (!importedState) return;
    state = normalizeState(importedState);
    localStorage.setItem('rewire_state', JSON.stringify(state));
    init();
    setServerSyncStatus('Server sync: loaded existing server data.');
  } catch {
    setServerSyncStatus('Server sync: backend endpoint not available. Using local data.');
  }
}
